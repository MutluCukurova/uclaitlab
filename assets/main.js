    ...
    if (tabs.length) {
      tabs.forEach(function (tab) {
        tab.addEventListener('click', function () {
          ...
          // Reveal any scroll-animated content in the newly shown panel (the
          // IntersectionObserver can't fire while a panel is display:none).
          var active = document.getElementById(target);
          if (active) active.querySelectorAll('.reveal:not(.in)').forEach(function (el) { el.classList.add('in'); });
        });
      });
    }
  });
